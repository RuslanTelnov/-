...
interface MonthSalesModalProps {
    isOpen: boolean
    onClose: () => void
    items: MonthSalesItem[]
}

type SortField = 'name' | 'quantity' | 'avgSalePrice' | 'costPrice' | 'profit' | 'margin'
type SortDirection = 'asc' | 'desc'

export function MonthSalesModal({ isOpen, onClose, items }: MonthSalesModalProps) {
    ...
    const totalSales = filteredAndSortedItems.reduce((sum, item) => sum + (item.avgSalePrice * item.quantity), 0)
    const totalProfit = filteredAndSortedItems.reduce((sum, item) => sum + item.profit, 0)
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
            <div className="bg-[#0B1121] border border-fintech-border rounded-3xl w-full max-w-6xl max-h-[90vh] flex flex-col shadow-2xl relative overflow-hidden">
                {/* Background Glows */}
                <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-transparent via-fintech-aqua to-transparent opacity-50"></div>
                <div className="absolute -top-20 -right-20 w-64 h-64 bg-fintech-aqua/10 blur-[80px] rounded-full pointer-events-none"></div>
                ...
                <div className="p-6 border-b border-fintech-border flex justify-between items-center z-10 bg-[#0B1121]/80 backdrop-blur-md">
                    <div className="flex items-center gap-4">
                        <div className="p-3 rounded-xl bg-fintech-aqua/10 text-fintech-aqua">
                            <DollarSign className="w-6 h-6" />
                        </div>
                        <div>
                            <h2 className="text-2xl font-bold text-white tracking-tight">Продажи за месяц</h2>
                            <p className="text-fintech-text-muted text-sm">Детализация по проданным товарам</p>
                        </div>
                    </div>
                    <button
                        onClick={onClose}
                ...
                <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6 items-center bg-[#0B1121]">
                    <div className="relative">
                        ...
                        <input
                            ...
                            className="w-full bg-white/5 border border-fintech-border rounded-xl py-3 pl-12 pr-4 text-white placeholder-fintech-text-muted focus:outline-none focus:border-fintech-aqua/50 transition-colors"
                        />
                    </div>
                    <div className="flex justify-end items-center gap-6">
                        <div className="text-right">
                            <p className="text-sm text-fintech-text-muted uppercase tracking-wider">Итого продажи</p>
                            <p className="text-xl font-bold text-white">
                                {totalSales.toLocaleString('ru-RU', { maximumFractionDigits: 0 })} ₸
                            </p>
                        </div>
                        <div className="text-right">
                            <p className="text-sm text-fintech-text-muted uppercase tracking-wider">Итого прибыль</p>
                            <p className="text-xl font-bold text-fintech-aqua text-glow">
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead className="sticky top-0 bg-[#0B1121] z-10 shadow-sm">
                            <tr className="border-b border-fintech-border/50">
                                ...
                                <th className="p-4 font-medium text-fintech-text-muted text-sm text-right cursor-pointer hover:text-white transition-colors group" onClick={() => handleSort('avgSalePrice')}>
                                    <div className="flex items-center justify-end gap-2">Цена прод. <SortIcon field="avgSalePrice" /></div>
                                </th>
                                ...
                                <th className="p-4 font-medium text-fintech-text-muted text-sm text-right cursor-pointer hover:text-white transition-colors group" onClick={() => handleSort('margin')}>
                                    <div className="flex items-center justify-end gap-2">Маржа <SortIcon field="margin" /></div>
                                </th>
                                <th className="p-4 pr-8 font-medium text-fintech-text-muted text-sm text-right cursor-pointer hover:text-white transition-colors group" onClick={() => handleSort('profit')}>
                            ...
                            {filteredAndSortedItems.length > 0 ? (
                                filteredAndSortedItems.map((item) => (
                                    <tr key={item.id} className="hover:bg-white/5 transition-colors group">
                                        ...
                                        <td className="p-4 text-right font-mono text-white">{item.avgSalePrice.toLocaleString('ru-RU')} ₸</td>
                                        <td className="p-4 text-right font-mono text-fintech-text-muted">{item.costPrice.toLocaleString('ru-RU')} ₸</td>
                                        <td className="p-4 text-right font-mono">
                                            <span className={`px-2 py-1 rounded-md text-xs ${item.margin >= 30 ? 'bg-fintech-green/10 text-fintech-green' : item.margin >= 15 ? 'bg-yellow-500/10 text-yellow-500' : 'bg-red-500/10 text-red-500'}`}>
                                                {item.margin.toFixed(1)}%
                                            </span>
                                        </td>
                                        <td className="p-4 pr-8 text-right font-mono font-bold text-fintech-aqua">
