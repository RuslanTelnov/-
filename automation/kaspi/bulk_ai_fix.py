...
if not anthropic_key:
    ...
    res = supabase.table('client_configs').select('anthropic_api_key').limit(1).execute()
...
def get_rejected_products():
    ...
    res = supabase.table('wb_search_results').select("*").eq("kaspi_status", "rejected").execute()
    return res.data
...
def apply_fix(product, suggestion):
    ...
    try:
        supabase.table('wb_search_results').update(update_data).eq("id", product['id']).execute()
