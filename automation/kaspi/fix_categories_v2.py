...
def fix_categories(article_ids):
    ...
    def update_product(p_id, name, desc):
        ...
        if cat_name:
            ...
            # Update DB
            # Fetch current specs
            current = supabase.table('wb_search_results').select('specs').eq('id', p_id).execute()
            ...
            try:
                supabase.table('wb_search_results').update(data).eq("id", p_id).execute()
    ...
    # Fetch
    try:
        resp = supabase.table('wb_search_results').select("*").in_("id", article_ids).execute()
        for p in resp.data:
